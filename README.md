...
2. **Build** the Lambda zip (`extensions/<extension>/package/lambda_deployment.zip`)
  ...
  ```bash
  python3 dev/extensions-service/run.py <extension> deploy deploy \
    --profile <aws-profile>
  ```
...
2. **Build** the Lambda zip and the ECS Docker image (heavy dependencies)
  ...
  ```bash
  python3 dev/extensions-service/run.py <extension> build --large
  ```
...
3. **Deploy** Lambda (zip) and push the ECS image (ECR + task definition)
  ...
  python3 dev/extensions-service/run.py <extension> deploy push \
    --profile <aws-profile>
  ```
...
Export env vars for `launcher/vars.json` and write `state/<extension>/lambda_env_export.json`:
...
Delete all provisioned AWS resources and local `state/<extension>/`. Use flag `--keep-logs` to Keep CloudWatch log groups when tearing down:
...
Stage 2 needs `dev/extensions-service/state/<extension>/deploy_input.json`. Can be generated by `bootstrap/install.py`, and must have shape as defined in `extensions-service\state\schemas\deploy_input.schema.json`
...
|--------------|---------|
| `lambda_deployment.zip` | `deploy deploy` / `deploy update` |
| `<ext>-ecs-builder:latest` image | `deploy push` |

### `deploy deploy` / `deploy update` / `deploy undeploy`

Uses `deploy_input.json` and `lambda_deployment.zip`. Subcommands map to `deploy_as_a_service.sh` (creates/updates the `{ext}-handlers` function).
...
- **`extensions/<name>/package/`** — handler code and `pyproject.toml`
- **`extensions/<name>/package/handlers_config.json`** — routing for `lambda_router.py`
- **`dev/extensions-service/state/<name>/`** — local state (gitignored except `schemas/`)
