...
def main() -> int:
    if len(sys.argv) < 2:
        ...
        print("                   shows ECS is provisioned. Flags: --wheelhouse DIR,", file=sys.stderr)
        print("                   --assets DIR, --packages a,b (required),", file=sys.stderr)
        print("                   --no-ecs, --local (ARM64 tag).", file=sys.stderr)
        print("                   --large / --extensions are not supported yet.", file=sys.stderr)
